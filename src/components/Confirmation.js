function ListedItems ({ result }) {
  return (Object.entries(result).map(([key, value]) => {
    ...
    return (
        <tr>
            <td key={key+amount} style={{ textAlign: "right" }}>{amount}</td>
            <td key={key} style={{ textAlign: "left" }}>{key}</td>
            <td key={key+price} style={{ textAlign: "right" }}>{price} kr</td>
        </tr>
    );})
  )
}
      ...
      <div style={{display: "flex", justifyContent: "center"}}>
          <table>
            <tbody>
              ...
              <tr>
                <td></td><td></td><td style={{ textAlign: "right", borderTop: "1px solid" }}>{getTotal( meals )} kr</td>  
              </tr>                    
            </tbody>
          </table>
      </div>
...
function ConfirmWrapper({ children }) {
  return (
    <div id="confirmtWrapper" style={{ 
    position: "relative",
